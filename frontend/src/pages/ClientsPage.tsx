            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="bg-primary-500 text-white px-4 py-2 rounded-md flex items-center gap-2 hover:bg-primary-600 transition-colors w-full md:w-auto justify-center"
                    >
                        <Plus size={20} />
                        Nuevo Cliente
                    </button>
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4">
                ...
                <div className="relative flex-1">
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 border border-primary-200 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500"
                    />
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-white">
                            <tr>
                                <th className="px-6 py-3 text-xs font-medium text-primary-600 uppercase tracking-wider">Cliente</th>
                                <th className="px-6 py-3 text-xs font-medium text-primary-600 uppercase tracking-wider">Contacto</th>
                                <th className="px-6 py-3 text-xs font-medium text-primary-600 uppercase tracking-wider hidden md:table-cell">Documento</th>
                                <th className="px-6 py-3 text-xs font-medium text-primary-600 uppercase tracking-wider hidden lg:table-cell">Nacionalidad</th>
                                <th className="px-6 py-3 text-xs font-medium text-primary-600 uppercase tracking-wider hidden lg:table-cell">Recomendado Por</th>
                                {isAdmin && (
                                    <th className="px-6 py-3 text-xs font-medium text-primary-600 uppercase tracking-wider text-right">Acciones</th>
                                )}
                            </tr>
                        </thead>
                                ...
                                filteredClients?.map((client) => (
                                    <tr key={client.id} className="hover:bg-primary-50 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <div className="text-sm text-primary-600 md:hidden">{client.cedula}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-gray-900">{client.phone}</div>
                                            {client.address && (
                                                <div className="text-xs text-primary-600 truncate max-w-[150px]">{client.address}</div>
                                            )}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap hidden md:table-cell text-primary-600">
                                            {client.cedula}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap hidden lg:table-cell text-primary-600">
                                            {client.nationality}
                                        </td>
